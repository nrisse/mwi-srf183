...
if __name__ == '__main__':
    ...
    for i, channel in enumerate(mwi.channels_int):
        ...
        axes[i].plot(sen.data.frequency*1e-3, sen.data.raw.sel(channel=channel),
    ...
    for i, channel in enumerate(mwi.channels_int):
        ...
        # MWI-RX183_DSB_Matlab.xlsx dataset
        axes[i].plot(sen_dsb.data.frequency*1e-3, sen_dsb.data.lino.sel(channel=channel)*100,
                     color='k', linewidth=0.9, label='MWI-RX183_DSB_Matlab.xlsx')
        
        # MWI-RX183_Matlab.xslx
        axes[i].plot(sen.data.frequency*1e-3,
                     sen.data.lino.sel(channel=channel)*100,
                     color='blue', alpha=0.5, linewidth=0.9, label='MWI-RX183_Matlab.xlsx')
    ...
    for i, channel in enumerate(mwi.channels_int):
        ...
        # MWI-RX183_Matlab.xslx
        axes[i].plot(sen.data.frequency, sen.data.lino.sel(channel=channel)*100/2,
                     color='blue', alpha=0.5, linewidth=0.9, label='MWI-RX183_Matlab.xlsx')
    ...
    for i, channel in enumerate(mwi.channels_int):
        ...
        # MWI-RX183_DSB_Matlab.xlsx dataset
        axes[i].plot(sen_dsb.data.frequency*1e-3, sen_dsb.data.raw.sel(channel=channel), color='k', linewidth=1, zorder=2)
    ...
    sen_dsb.data['tophat'] = xr.zeros_like(sen_dsb.data.lino)
    ...
    for i, channel in enumerate(mwi.channels_int):
        ...
        sen_dsb.data['tophat'][ix_in, i] = 1
    
    sen_dsb.data['tophat'] = sen_dsb.data['tophat']/sen_dsb.data['tophat'].sum('frequency')
    ...
    for i, channel in enumerate(mwi.channels_int):
        for j in range(2):
            ...
            ax.fill_between(x=sen_dsb.data.frequency*1e-3,
                            y1=sen_dsb.data.tophat.sel(channel=channel)*100, 
